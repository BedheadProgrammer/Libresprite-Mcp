...
# ---------------------------------------------------------------------------
# Pure-Python PNG reader (stdlib only — no Pillow dependency)
# ---------------------------------------------------------------------------


def _get_latest_sprite_path() -> str | None:
    """Return the path to the most recently generated sprite PNG, or *None*."""
    try:
        files = [
            f
...
def _read_png_info(png_path: str) -> dict:
    """Read basic metadata from a PNG file using only the stdlib.

    Returns a dict with keys: ``width``, ``height``, ``bit_depth``,
    ``color_type``, ``color_mode``, and ``filename``.
    """
    import struct
...
def _read_png_pixels(png_path: str) -> tuple[bytes, int, int]:
    """Decode a PNG file into raw RGBA pixel data.

    Returns ``(rgba_bytes, width, height)`` where *rgba_bytes* has
    exactly ``width * height * 4`` bytes in row-major RGBA order.

    Only 8-bit non-interlaced PNGs are supported (covers virtually all
    sprites generated by LibreSprite and ``_create_blank_png``).
    """
    import struct
    import zlib
    ...
    with open(png_path, "rb") as f:
        ...
        while True:
            ...
            if chunk_type == b"IHDR":
                (width, height, bit_depth, color_type,
                 _, _, _) = struct.unpack(">IIBBBBB", data)
    ...
    if bit_depth != 8:
        raise ValueError(
            f"Only 8-bit PNGs are supported, got {bit_depth}-bit."
        )
    ...
    for _ in range(height):
        filter_byte = raw_data[offset]
        offset += 1
        row = bytearray(raw_data[offset : offset + stride])
        offset += stride

        # Reverse the PNG filter
        if filter_byte == 1:  # Sub
            ...
            for i in range(stride):
                ...
                if pa <= pb and pa <= pc:
                    ...
                    pr = c
                row[i] = (row[i] + pr) & 0xFF
        # filter_byte == 0 → None, no action needed
        ...
        if color_type == 6:  # RGBA
            ...
            for i in range(0, stride, 3):
                ...
                pixels.append(255)
        elif color_type == 0:  # Grayscale → RGBA
            for i in range(stride):
                pixels.extend([row[i], row[i], row[i], 255])
        elif color_type == 4:  # Grayscale+Alpha → RGBA
            for i in range(0, stride, 2):
                pixels.extend([row[i], row[i], row[i], row[i + 1]])
        elif color_type == 3:  # Indexed → RGBA
            for i in range(stride):
                idx = row[i]
                if idx < len(palette):
                    r, g, b = palette[idx]
...
def get_sprite_info() -> str:
    ...
    # Docker mode — read from the most recently saved sprite PNG.
    sprite_path = _get_latest_sprite_path()
    if sprite_path is None:
        return "No sprites have been generated yet. Create a sprite first."
    try:
        info = _read_png_info(sprite_path)
        return (
            f"width:{info['width']}\n"
            f"height:{info['height']}\n"
            f"colorMode:{info['color_mode']}\n"
            f"layerCount:1\n"
            f"filename:{info['filename']}"
        )
    except Exception as exc:
        return f"Error reading sprite info: {exc}"
...
def get_pixel_data(x: int, y: int, width: int = 1, height: int = 1) -> str:
    ...
    """
    # Cap the read area to avoid overwhelming output.
    max_pixels = 4096
    ...
    if sprite_path is None:
        return "No sprites have been generated yet. Create a sprite first."
    try:
        rgba_data, img_w, img_h = _read_png_pixels(sprite_path)
    except Exception as exc:
        return f"Error reading pixel data: {exc}"
    ...
    for py in range(y, y + height):
        for px in range(x, x + width):
            if 0 <= px < img_w and 0 <= py < img_h:
                ...
                lines.append(f"({px},{py}):rgba({r},{g},{b},{a})")
            else:
                lines.append(f"({px},{py}):out-of-bounds")
    return "\n".join(lines)
    

@mcp.tool()
def screenshot() -> str | list[TextContent | ImageContent]:
    ...
    **Recommended workflow:**
    ...
    if MODE == "relay":
        ...
        if "__MCP_IMGDATA__:" in result:
            ...
            try:
                ...
                return [
                    TextContent(type="text", text="Current sprite preview:"),
    ...
    return [
        TextContent(type="text", text=f"Latest sprite preview ({files[0]}):"),
        ImageContent(type="image", data=b64, mimeType="image/png"),
    ]
